...
# instructions
@cache
def pathfind(code: str, target, depth=0):
    if depth == 0:
    ...
    for next_ in code:
        ...
        updo = "".join("v" if man_y > 0 else "^" for _ in range(abs(man_y)))
        leri = "".join(">" if man_x > 0 else "<" for _ in range(abs(man_x)))
        if depth == 0 and c_coords[0] == 3 and n_coords[1] == 0:
            step = updo + leri
        elif depth == 0 and c_coords[1] == 0 and n_coords[0] == 3:
            step = leri + updo
        elif depth > 0 and c_coords == (1,0):
            step = leri + updo
        elif depth > 0 and n_coords == (1,0):
            step = updo + leri
        elif man_x < 0:
            step = leri + updo
        else:
        ...
        loc = next_
    if depth < target:
        return pathfind(output, target, depth+1)
    return len(output)
...
p1_answer = sum(int(code.strip("A")) * pathfind(code, 2) for code in codes)
